...
PHLEX_REGISTER_ALGORITHMS(m, config)
{
  ...
  // ---------------------------------------------------------------
  m.transform(
     "find_hits_with_gaussians_design2",
     [cfg = main_cfg(config),
      cand_hit_standard_vec = make_cand_hit_standard_vec(config),
      peak_fitter_mrqdt = make_peak_fitter_mrqdt(config),
      hit_filter_alg = make_hit_filter_alg(config)](examples::wire_roi_data const& roi_data) {
       return examples::find_hits_with_gaussians_design2(
         cfg, roi_data, cand_hit_standard_vec, *peak_fitter_mrqdt, *hit_filter_alg);
     },
     concurrency::unlimited)
  ...
  m.fold("fold_roi_hits_design2", examples::fold_roi_hits_design2, concurrency::serial, layer_wire)
    .input_family(
      product_selector{.creator = "find_hits_with_gaussians_design2", .layer = layer_roi});

  // ---------------------------------------------------------------
  // Outer fold:  wire -> spill  (collects hits from all wires)
  // ---------------------------------------------------------------
  m.fold("fold_hits_into_vector_design2",
         examples::fold_hits_into_vector_design2,
         concurrency::serial,
