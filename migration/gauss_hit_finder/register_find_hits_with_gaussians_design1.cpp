...
namespace {
  ...
  std::vector<std::shared_ptr<examples::CandHitStandard>> make_cand_hit_standard_vec(
    ...
    for (auto const& key : finder_configs.keys()) {
      ...
      if (plane >= result.size()) {
        std::cerr << "Error: plane number " << plane
                  << " is out of range for cand_hit_standard_vec of size " << result.size()
                  << std::endl;
...
PHLEX_REGISTER_ALGORITHMS(m, config)
{
  ...
  m.transform(
     "find_hits_with_gaussians_design1",
     [cfg = main_cfg(config),
      cand_hit_standard_vec = make_cand_hit_standard_vec(config),
      peak_fitter_mrqdt = make_peak_fitter_mrqdt(config),
      hit_filter_alg = make_hit_filter_alg(config)](recob::Wire const& wire) {
       return examples::find_hits_with_gaussians_design1(
         cfg, wire, cand_hit_standard_vec, *peak_fitter_mrqdt, *hit_filter_alg);
     },
     concurrency::unlimited)
    .input_family(
      product_selector{.creator = "unfold_wire_vector_design1", .layer = layer_unfold_output});

  m.fold("fold_hits_into_vector_design1",
         examples::fold_hits_into_vector_design1,
         concurrency::serial,
         layer_unfold_input)
    .input_family(product_selector{.creator = "find_hits_with_gaussians_design1",
                                   .layer = layer_unfold_output});
}
