...
bool write_wires_to_file(std::vector<recob::Wire> const& wires, std::string const& filename)
{
  ...
  out.write(reinterpret_cast<char const*>(&num_wires), sizeof(num_wires));

  for (auto const& wire : wires) {
    // Write channel ID
    raw::ChannelID_t channel = wire.Channel();
    out.write(reinterpret_cast<char const*>(&channel), sizeof(channel));

    // Write view
    geo::View_t view = wire.View();
    out.write(reinterpret_cast<char const*>(&view), sizeof(view));

    // Write sparse vector data
    auto const& signal_roi = wire.SignalROI();
    size_t nominal_size = signal_roi.size();
    out.write(reinterpret_cast<char const*>(&nominal_size), sizeof(nominal_size));

    // Write number of ranges
    size_t num_ranges = std::distance(signal_roi.begin_range(), signal_roi.end_range());
    out.write(reinterpret_cast<char const*>(&num_ranges), sizeof(num_ranges));
    ...
    for (auto range_it = signal_roi.begin_range(); range_it != signal_roi.end_range(); ++range_it) {
      ...
      out.write(reinterpret_cast<char const*>(&last), sizeof(last));

      // Write number of values
      size_t num_values = range_it->size();
      out.write(reinterpret_cast<char const*>(&num_values), sizeof(num_values));

      // Write values
      if (num_values > 0) {
        // Create a temporary vector to hold the values
        std::vector<float> values(range_it->begin(), range_it->end());
        out.write(reinterpret_cast<char const*>(values.data()), num_values * sizeof(float));
      }
    }
  }
  out.close();
...
std::optional<std::vector<recob::Wire>> read_wires_from_file(std::string const& filename)
{
  ...
  for (size_t i = 0; i < num_wires; ++i) {
    ...
    for (size_t j = 0; j < num_ranges; ++j) {
      ...
      if (num_values > 0) {
        in.read(reinterpret_cast<char*>(values.data()), num_values * sizeof(float));
        if (!in)
          return std::nullopt;
      }
